...
public class PaymentService {
    ...
    public void initUserBalanceInDB() {
        userBalanceRepository.saveAll(Stream.of(new UserBalance(101, 100),
                new UserBalance(102, 30),
                new UserBalance(103, 15),
                new UserBalance(104, 50),
                new UserBalance(105, 5)).collect(Collectors.toList()));
    }
    ...
    public boolean verifyId(String utenteIdent) {
        if (userBalanceRepository.findById(Integer.parseInt(utenteIdent)).isPresent()) {
            return true;
        }
        else {
            return false;
